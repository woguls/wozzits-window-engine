// src/engine/assets/csv/csv.cpp

#include <engine/assets/csv/csv.h>
#include <engine/assets/type_extensions.h>

namespace wz::engine::assets {

    CSVTable::CSVTable()
    {
        slots_.emplace_back(); // sentinel: epoch=0, occupied=false, id=0 is invalid
    }

    wz::asset::ResourceHandle CSVTable::add(CSVData data)
    {
        const uint32_t id = static_cast<uint32_t>(slots_.size());

        Slot& slot = slots_.emplace_back();
        slot.epoch    = 1;
        slot.occupied = true;
        slot.data     = std::move(data);

        return wz::asset::ResourceHandle{
            .id    = id,
            .epoch = slot.epoch,
            .type  = kAssetTypeCSVTable,
        };
    }

    const CSVData* CSVTable::get(wz::asset::ResourceHandle handle) const
    {
        if (handle.id >= static_cast<uint32_t>(slots_.size()))
            return nullptr;

        const Slot& slot = slots_[handle.id];

        if (!slot.occupied || slot.epoch != handle.epoch)
            return nullptr;

        return &slot.data;
    }

    void CSVTable::destroy()
    {
        slots_.clear();
        slots_.emplace_back(); // restore sentinel
    }

} // namespace wz::engine::assets
