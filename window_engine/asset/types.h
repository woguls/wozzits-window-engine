...
struct AssetKeyHash {
    size_t operator()(const AssetKey& k) const noexcept {
        // XOR-mix each 64-bit word using a Fibonacci multiplier.
...
enum class AssetType : uint16_t {
    ...
    Audio    = 5,
    Font     = 6,
    // Extend as needed. Values are stable across sessions (serialized in keys).
};

// ─── SchemaID ─────────────────────────────────────────────────────────────────
// Identifies the schema governing how an asset is decoded and compiled.
// Stored as a hash so schemas can be registered dynamically without a central
// enum — hash the schema name string at startup.
...
enum class AssetStage : uint8_t {
    ...
    Compiled     = 2,   // live GPU resource, ready for the renderer
};

// ─── GPUHandle ────────────────────────────────────────────────────────────────
// Opaque reference to a renderer-owned GPU resource.
...
// ─── AssetIR ──────────────────────────────────────────────────────────────────
// Intermediate representation: decoded asset data awaiting GPU upload.
...
//   Compiled     → GPUHandle         live GPU resource (mirrored from cache)
//
// The dependency list (depends_on in the design doc) is NOT stored here.
// It is encoded structurally as DAG edges: parent = prerequisite.
